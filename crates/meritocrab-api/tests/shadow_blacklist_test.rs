...
impl meritocrab_llm::LlmEvaluator for SpamEvaluator {
    async fn evaluate(
        &self,
        _content: &str,
        _context: &meritocrab_llm::EvalContext,
...
#[async_trait::async_trait]
impl meritocrab_llm::LlmEvaluator for TrackingEvaluator {
    async fn evaluate(
        &self,
        content: &str,
        _context: &meritocrab_llm::EvalContext,
...
async fn setup_test_state_with_evaluator(
    evaluator: Arc<dyn meritocrab_llm::LlmEvaluator>,
) -> AppState {
    sqlx::any::install_default_drivers();
...
#[allow(dead_code)]
fn create_test_pr_event(user_id: i64, username: &str) -> meritocrab_github::PullRequestEvent {
...
async fn test_auto_blacklist_at_threshold() {
    ...
    use meritocrab_core::apply_credit;
    use meritocrab_db::contributors::update_credit_score;
