...
async fn test_credit_override_applies_delta() {
    ...
    let contributor = meritocrab_db::contributors::create_contributor(
        &state.db_pool,
        12345,
        "test-owner",
        "test-repo",
    ...
    // Apply credit delta
    let new_score = meritocrab_core::apply_credit(contributor.credit_score, 15);
    assert_eq!(new_score, 115);

    // Update contributor
    meritocrab_db::contributors::update_credit_score(&state.db_pool, contributor.id, new_score)
        .await
...
async fn test_credit_override_triggers_auto_blacklist() {
    ...
    if new_score <= state.repo_config.blacklist_threshold {
        meritocrab_db::contributors::set_blacklisted(&state.db_pool, contributor.id, true)
            .await
...
async fn test_credit_events_logged_for_override() {
    ...
    // Log a credit event
    let event = meritocrab_db::credit_events::insert_credit_event(
        &state.db_pool,
        contributor.id,
        "manual_adjustment",
        10,
...
async fn test_list_credit_events_for_contributor() {
    ...
    for i in 0..5 {
        meritocrab_db::credit_events::insert_credit_event(
            &state.db_pool,
            contributor.id,
            "comment",
            i,
    ...
    // List events (last 5)
    let events = meritocrab_db::credit_events::list_events_by_contributor(
        &state.db_pool,
        contributor.id,
        5,
        0,
