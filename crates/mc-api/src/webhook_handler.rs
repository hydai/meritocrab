...
use rand::Rng;
use mc_core::{check_blacklist, check_pr_gate, calculate_delta_with_config, apply_credit, EventType, GateResult};
use mc_db::{
...
use mc_github::{PullRequestEvent, IssueCommentEvent, PullRequestReviewEvent};
...
async fn handle_credit_check(
    ...
    let contributor_opt = if let Ok(github_user_id) = target_username.parse::<i64>() {
        mc_db::contributors::get_contributor(&state.db_pool, github_user_id, &repo_owner, &repo_name).await?
    } else {
...
async fn evaluate_and_apply_credit(
    ...
    let contributor = mc_db::contributors::get_contributor(&state.db_pool, user_id, &repo_owner, &repo_name)
        .await?
