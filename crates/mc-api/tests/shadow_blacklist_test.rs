use mc_api::{state::AppState, OAuthConfig};
use mc_core::{config::QualityLevel, RepoConfig};
use mc_db::{
    contributors::{create_contributor, get_contributor, set_blacklisted},
    credit_events::list_events_by_contributor,
};
use mc_github::{GithubApiClient, WebhookSecret};
use mc_llm::Evaluation;
...
impl mc_llm::LlmEvaluator for SpamEvaluator {
    async fn evaluate(
        &self,
        _content: &str,
        _context: &mc_llm::EvalContext,
    ) -> Result<mc_llm::Evaluation, mc_llm::LlmError> {
        Ok(Evaluation {
...
#[async_trait::async_trait]
impl mc_llm::LlmEvaluator for TrackingEvaluator {
    async fn evaluate(
        &self,
        content: &str,
        _context: &mc_llm::EvalContext,
...
async fn setup_test_state() -> AppState {
    ...
    let llm_evaluator = Arc::new(mc_llm::MockEvaluator::new());
...
async fn setup_test_state_with_evaluator(
    evaluator: Arc<dyn mc_llm::LlmEvaluator>,
) -> AppState {
    sqlx::any::install_default_drivers();
...
#[allow(dead_code)]
fn create_test_pr_event(user_id: i64, username: &str) -> mc_github::PullRequestEvent {
...
async fn test_auto_blacklist_when_credit_drops_to_zero() {
    ...
    use mc_core::{apply_credit, calculate_delta_with_config, EventType};
    use mc_db::{contributors::update_credit_score, credit_events::insert_credit_event};
    use mc_llm::{ContentType, EvalContext};
...
async fn test_auto_blacklist_at_threshold() {
    ...
    use mc_core::apply_credit;
    use mc_db::contributors::update_credit_score;
